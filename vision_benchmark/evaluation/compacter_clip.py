...
from ..evaluation.clip_load import compacter_load
...
def get_cls_model(config, feature_type='image'):
    ...
    model, _ = compacter_load(config.MODEL.NAME, jit=False)
...
def compacter_clip(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config):
