...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.add_adapter is not None:
            x = x + self.adapter(self.mlp(self.ln_2(x)), self.mlp(self.ln_2(x)))[0]
        else:
...
def build_adapter_model(state_dict: dict):
