...
class MultiheadAttention(nn.Module):
    ...
    def adapter_forward(self, x, weight_1, weight_2, g_weight=None):
        ...
        if g_weight is not None:
            ...
            result = result.view(result.shape[0], result.shape[1], result.shape[2] // self.lora_moe_group, self.lora_moe_group) * g.unsqueeze(-1)
    ...
    def multi_head_attention_forward(
        ...
        q_delta = self.adapter_forward(lora_input, self.q_proj_adapter1.weight, self.q_proj_adapter2.weight, g_weight=self.q_moe_adapter1).reshape(B*self.num_heads, N, C // self.num_heads)
        v_delta = self.adapter_forward(lora_input, self.v_proj_adapter1.weight, self.v_proj_adapter2.weight, g_weight=self.v_moe_adapter1).reshape(B*self.num_heads, N, C // self.num_heads)
        # v_delta = self.adapter_forward(lora_input, self.v_proj_adapter1.weight, self.v_proj_adapter2.weight, g_weight=self.v_moe_adapter1).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        # return self.resblock(self.resblocks(x))
...
def build_lora_model(state_dict: dict):
