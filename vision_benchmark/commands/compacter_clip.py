...
from vision_benchmark.evaluation.compacter_clip import *
...
def main():
    ...
    best_acc, model_info = compacter_clip(train_dataloader, val_dataloader, test_dataloader, args.no_tuning, args.lr, args.l2, config)
    ...
    if args.save_predictions:
        ...
        with open(os.path.join(prediction_folder, f'seed{config.DATASET.RANDOM_SEED_SAMPLING}_{config.DATASET.DATASET}.txt' ) , 'w') as outfile:
            outfile.write(f'best acc is:{best_acc}, num_params is:{num_params}, n_trainable_params is:{num_trainable_params}, backbone_params is:{n_backbone_params}.')
